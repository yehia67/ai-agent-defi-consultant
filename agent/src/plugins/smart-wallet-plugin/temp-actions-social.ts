...
export const addSocialRecoveryAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ...
        const guardians = (message.content?.data as {guardians:string[]})?.guardians || [];
        ...
        try {
            ...
            if (callback) {
                try {
                    ...
                    logger.info('SOCIAL_RECOVERY callback sent successfully');
                } catch (callbackError) {
                    logger.debug('SOCIAL_RECOVERY callback error:', callbackError);
                }
            }
            ...
            logger.debug('Error in addSocialRecoveryAction:', error);
            ...
            if (callback) {
                try {
                    ...
                    logger.info('SOCIAL_RECOVERY error callback sent successfully');
                } catch (callbackError) {
                    logger.debug('SOCIAL_RECOVERY error callback error:', callbackError);
                }
            }
